...
function drawProjectiles() {
  // resetConsole();
  for (let i = 0; i < anglesAndVelocities.length; i++) {
    ...
    // addToConsole(`Tr. ${i}/A: V0: ${velocity} m/s, θ: ${Math.floor(radToDeg(angleA) * 100) / 100} deg, T: ${Math.floor(timeA * 100) / 100}s\n`);
    // addToConsole(`Tr. ${i}/B: V0: ${velocity} m/s, θ: ${Math.floor(radToDeg(angleB) * 100) / 100} deg, T: ${Math.floor(timeB * 100) / 100}s\n`);    
  }
}
...
function calculatePoints(velocity, angle, time) {
  ...
  for (var i = 0; i < Math.ceil(time * 10); i++) {
    ...
    setInterval(drawPoint(point.x, canvas.height - point.y), 3000);
  }
}
